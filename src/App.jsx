...
function defaultSceneItemRotation(entry = {}, type = '') {
  ...
  if (signature.includes('bar') && (signature.includes('signa') || signature.includes('signal') || signature.includes('logo'))) return 0;
...
function ConfiguratorApp({ initialScene, isAdminViewer = false, forceReadOnly = false }) {
  ...
  const currentScenePayload = (status, clientStatus, overrides = {}) => {
    ...
    const options = {
      ...
      carpetColorCode: selectedCarpetColor.code || '',
      ...
      carpetFootprintColorCode: selectedCarpetFootprintColor.code || '',
      ...
      wallFabricColorCode: selectedWallFabricColor.code || '',
      ...
      reserveWallFabricColorCode: selectedReserveWallFabricColor.code || '',
...
function AdminDashboard({ user, adminProfile }) {
  ...
  const deleteAdminSalon = async (salon) => {
    if (!salon?.id) return;
    const confirmed = window.confirm(`Supprimer définitivement le salon ${salon.name} ?\n\nCette action supprime aussi ses packs, ses presets et ses liaisons Monday. Elle est refusée si des scènes sont encore liées au salon.`);
    if (!confirmed) return false;
    await deleteSalon(salon);
    await refreshSalons();
    setPresetSalonId((current) => (current === salon.id ? '' : current));
    return true;
  };
...
function AdminPresetsView({ salons, assets, initialSalonId, onSalonChanged }) {
  ...
  const removePreset = async (entry) => {
    ...
    setActionState({ loadingPack: '', savingBoardPack: '', savingBasePack: '', deletingPresetId: deleteKey, message: '', error: '' });
    try {
  ...
  const updateCeilingMountedOption = (checked) => {
    setDraft({
      ...
      dimensions: {
        ...
        ceilingBottomY: checked ? Number(draft.dimensions?.ceilingBottomY ?? ceilingObjectBottomY) : draft.dimensions?.ceilingBottomY,
        ...(checked ? { mountType: 'floor', isTelevision: false, isLedSpotOption: false, wallTopSnap: false } : {}),
      },
    });
  };

  const updateCeilingBottomDistance = (value) => {
    const distance = Number(value);
    updateAssetBehavior({ ceilingBottomY: Number.isFinite(distance) ? Math.max(0, distance) : ceilingObjectBottomY });
  };
        ...
        <section className="asset-behavior-settings">
          ...
          <label className="asset-toggle-row">
            ...
            <span>
              ...
              <small>Place l'objet à une distance réglable entre le sol et le bas de l'enseigne, avec déplacement libre en X/Z.</small>
            </span>
          </label>
          {draftCeilingMounted && (
            <label>
              <span>Distance sol → bas de l'objet (m)</span>
              <input
                type="number"
                min="0"
                step="0.05"
                value={draft.dimensions?.ceilingBottomY ?? ceilingObjectBottomY}
                onChange={(event) => updateCeilingBottomDistance(event.target.value)}
              />
            </label>
          )}
          <label className="asset-toggle-row">
...
function floorItemBaseY(item = {}, entry = null) {
  ...
  return Number.isFinite(y) ? Math.max(0, y) : ceilingObjectBottomY;
}
